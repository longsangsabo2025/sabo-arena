...
/// Update display_name from full_name in users table

class UpdateDisplayNameFromFullName {
  static String newSupabaseUrl = '';
  static String newSupabaseServiceKey = '';

  static Future<void> update({bool dryRun = true}) async {
    print('🔄 UPDATING DISPLAY_NAME FROM FULL_NAME...\n');
    ...
    for (var i = 0; i < users.length && i < 10; i++) {
      ...
      print('   Current display_name: ${user['display_name']}');
      print('   full_name: ${user['full_name']}');
      print('   → Will set display_name to: ${user['full_name']}');
      print('');
    }
    if (users.length > 10) {
    ...
    if (!dryRun) {
      ...
      for (var i = 0; i < users.length; i++) {
        ...
        print('   Setting display_name to: $fullName');

        try {
          await _updateDisplayName(userId, fullName);
          successCount++;
          print('  ✅ Updated\n');
        } catch (e) {
          failCount++;
  ...
  static Future<List<Map<String, dynamic>>> _fetchAllUsers() async {
    final response = await http.get(
      Uri.parse(
        '$newSupabaseUrl/rest/v1/users?select=id,email,display_name,full_name',
      ),
      headers: {
  ...
  static Future<void> _updateDisplayName(
    String userId,
...
void main(List<String> args) async {
  try {
    ...
    await UpdateDisplayNameFromFullName.update(dryRun: !shouldUpdate);
  } catch (e) {
    print('\n❌ Update failed: $e');
    exit(1);
  }
