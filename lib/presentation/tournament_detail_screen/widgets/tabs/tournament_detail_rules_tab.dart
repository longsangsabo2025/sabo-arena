...
class TournamentDetailRulesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (tournament?.rules != null && tournament!.rules!.trim().isNotEmpty) {
      ...
      ProductionLogger.info('📋 Loaded ${rules.length} rules from tournament data', tag: 'TournamentDetailRulesTab');
    }
    
    // Try special rules if main rules is empty
    if (rules.isEmpty && tournament?.specialRules != null && tournament!.specialRules!.trim().isNotEmpty) {
      ...
      ProductionLogger.info('📋 Loaded ${rules.length} rules from special rules', tag: 'TournamentDetailRulesTab');
    }
    
    // Fallback to default rules if still empty
    if (rules.isEmpty) {
      rules = defaultRules;
      ProductionLogger.info('📋 Using ${rules.length} default fallback rules', tag: 'TournamentDetailRulesTab');
    }
