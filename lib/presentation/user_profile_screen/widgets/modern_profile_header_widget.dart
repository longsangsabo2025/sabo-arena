...
class _ModernProfileHeaderWidgetState extends State<ModernProfileHeaderWidget> {
  ...
  Widget _buildProfileInfoRow(BuildContext context) {
    // Use UserDisplayNameText for consistent display name handling
    final displayName =
        widget.userData["displayName"] as String? ?? "Player Name";
    final currentRankCode = widget.userData["currentRankCode"] as String?;
    final bool hasRank = currentRankCode != null && currentRankCode.isNotEmpty;
    final currentRankColor = hasRank
        ? SaboRankSystem.getRankColor(currentRankCode)
        : AppColors.textTertiary;
    final bio = widget.userData["bio"] as String? ?? "Chơi bida chuyên nghiệp";

    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Avatar hình vuông bo góc - Cột trái
        Container(
          width: 100,
          height: 100,
          decoration: BoxDecoration(
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              // Rank Badge - clickable if no rank
              GestureDetector(
                onTap: hasRank
                    ? null
                    : () => _showRankRegistrationSheet(context),
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: currentRankColor, width: 2),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
  ...
  Widget _buildCoverWithOverlay(BuildContext context) {
    // Display name is now handled by UserDisplayNameText
    final displayName =
        widget.userData["displayName"] as String? ?? "Player Name";
    final coverPhoto =
        widget.userData["coverPhoto"] as String? ??
        "https://images.pexels.com/photos/1040473/pexels-photo-1040473.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1";

    return GestureDetector(
      onTap: widget.onCoverPhotoTap,
      child: SizedBox(
        height: 250,
        width: double.infinity,
        child: Stack(
          ...
          children: [
            ...
            children: [
              ...
              Positioned(
                top: 16,
                right: 16,
                child: Icon(Icons.more_horiz, color: AppColors.textOnPrimary, size: 24),
              ),

              // Avatar hình vuông bo góc ở giữa
              Positioned(
                bottom:
                    -40, // Đẩy xuống dưới một nửa để nằm giữa cover và content
                left: 0,
                right: 0,
                child: Center(
                  child: Container(
              ...
              if (widget.onCoverPhotoTap != null)
                Positioned(
                  bottom: 16,
                  right: 16,
                  child: Container(
                    ...
                    child: Icon(Icons.edit, color: AppColors.textOnPrimary, size: 20),
                  ),
                ),
            ],
          ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: currentRankColor.withValues(alpha: 0.3),
              blurRadius: 8,
              offset: const Offset(0, 2),
            ),
          ],
        ...
        child: Row(
          ...
          children: [
            ...
            if (!hasRank) ...[
              const SizedBox(width: 8),
              Icon(Icons.arrow_forward_ios, color: currentRankColor, size: 14),
            ],
          ],
        ),
      ),
